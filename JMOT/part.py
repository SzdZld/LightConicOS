...
_PART_SIGNALS = {
    ...
    "get_variable": 416,
    "get_variable_list": 417,
    "part_loacl_to_eci":365,
    "part_eci_to_local":366
}
...
def part_loacl_to_eci(partID:int, vector:np.ndarray)->np.ndarray:
    ...
    rec = connect._send_message(f"true<<{_PART_SIGNALS['part_loacl_to_eci']}<<{partID}<<{v}")
    vec = extra.tuple2array(rec[0])
    return vec
def part_eci_to_local(partID:int, vector:np.ndarray)->np.ndarray:
    ...
    v = extra.array2tuple(vector)
    rec = connect._send_message(f"true<<{_PART_SIGNALS['part_eci_to_local']}<<{partID}<<{v}")
    vec = extra.tuple2array(rec[0])
    return vec
...
def get_variable_list(part_id:int, variable_name:str)->list:
    ...
    rec = connect._send_message(f"true<<{_PART_SIGNALS['get_variable_list']}<<{part_id}<<{variable_name}")
    return rec